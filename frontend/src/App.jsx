...
export default function App() {
  ...
  // ESTADOS EDIÇÃO TITULAR E AGREGADO
  ...
  const abrirModalEditar = (c) => {
    ...
    setFormData({
      ...
      data_entrada_af_atual: c.data_entrada_af_atual || '', data_saida_af_atual: c.data_saida_af_atual || '',
  ...
  const handleRemoverMembro = (membroId) => {
    axios.delete(`${API_BASE_URL}/api/membros/${membroId}`)
      .then(() => abrirModalFamilia(familiaSelecionada.id));
  };
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', padding: '24px', backgroundColor: '#f8fafc', minHeight: '100vh' }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={c.id}>
                  ...
                  <td style={{ padding: '12px' }}>● {c.estado_processo}</td>
                  <td style={{ padding: '12px' }}>{c.titular_acolhimento}</td>
                  <td style={{ padding: '12px', textAlign: 'center' }}>
                    <div style={{ display: 'flex', gap: '4px', justifyContent: 'center' }}>
                      {c.familia_id && <button onClick={() => abrirModalFamilia(c.familia_id)} style={{ padding: '4px 8px' }}>👁️</button>}
                      {isAdmin && (
                        <>
                          <button onClick={() => abrirModalEditar(c)} style={{ padding: '4px 8px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '4px' }}>✏️</button>
                          <button onClick={() => handleEliminar(c.id, c.nome)} style={{ padding: '4px 8px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '4px' }}>🗑️</button>
                        </>
                      )}
                    </div>
                  </td>
      ...
      {/* 📝 MODAL EXPANDIDO DE CRIAR / EDITAR CRIANÇA */}
      {modalFormAberto && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ backgroundColor: '#ffffff', padding: '28px', borderRadius: '16px', maxWidth: '800px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
            ...
            <form onSubmit={handleSubmitForm} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginTop: '16px' }}>
              ...
              <div style={{ gridColumn: 'span 3', marginTop: '10px' }}><label style={{ fontSize: '11px', fontWeight: 'bold' }}>Transição para:</label><input type="text" value={formData.transicao_para} onChange={e => setFormData({...formData, transicao_para: e.target.value})} placeholder="Ex: Adoção, Maioridade, Autonomia" style={{ width: '100%', padding: '6px', boxSizing: 'border-box' }} /></div>
      ...
      {/* 🏠 MODAL FICHA FAMÍLIA & AGREGADO (COM EDIÇÃO) */}
      {familiaSelecionada && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ backgroundColor: '#ffffff', padding: '28px', borderRadius: '16px', maxWidth: '800px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ marginTop: 0, color: '#0f172a' }}>🏠 Ficha do Titular e Agregado</h2>
              {isAdmin && !editandoTitular && (
                <button onClick={() => setEditandoTitular(true)} style={{ padding: '6px 12px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold' }}>
                  ✏️ Editar Dados do Titular
                </button>
              )}
            </div>

            {/* EDIÇÃO OU VISUALIZAÇÃO DO TITULAR */}
            {editandoTitular ? (
              <div style={{ backgroundColor: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #cbd5e1', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '12px' }}>
                <div><label>N.º Certificação</label><input type="text" value={formFamilia.no_certificacao || ''} onChange={e => setFormFamilia({...formFamilia, no_certificacao: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div><label>Nome Titular</label><input type="text" value={formFamilia.titular_nome || ''} onChange={e => setFormFamilia({...formFamilia, titular_nome: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div><label>Contacto</label><input type="text" value={formFamilia.contacto || ''} onChange={e => setFormFamilia({...formFamilia, contacto: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div><label>Email</label><input type="email" value={formFamilia.email || ''} onChange={e => setFormFamilia({...formFamilia, email: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div style={{ gridColumn: 'span 2' }}><label>Morada</label><input type="text" value={formFamilia.morada || ''} onChange={e => setFormFamilia({...formFamilia, morada: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div><label>NIF</label><input type="text" value={formFamilia.nif || ''} onChange={e => setFormFamilia({...formFamilia, nif: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div><label>NISS</label><input type="text" value={formFamilia.niss || ''} onChange={e => setFormFamilia({...formFamilia, niss: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div><label>SNS</label><input type="text" value={formFamilia.sns || ''} onChange={e => setFormFamilia({...formFamilia, sns: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
                <div><label>Data Revisão</label><input type="date" value={formFamilia.data_revisao || ''} onChange={e => setFormFamilia({...formFamilia, data_revisao: e.target.value})} style={{ width: '100%', padding: '6px' }} /></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', backgroundColor: '#f8fafc', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0', fontSize: '13px' }}>
                ...
                <div><strong>N.º Certificação:</strong> {familiaSelecionada.no_certificacao}</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', marginTop: '8px' }}>
              ...
              <tbody>
                {familiaSelecionada.membros_agregado.length === 0 ? (
                  <tr><td colSpan="6" style={{ padding: '12px', textAlign: 'center', color: '#94a3b8' }}>Sem outros membros registados.</td></tr>
                ) : (
                    ...
                    <tr key={m.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      {isAdmin && (
                        <td style={{ padding: '8px', textAlign: 'center' }}>
                          <button onClick={() => handleRemoverMembro(m.id)} style={{ padding: '2px 6px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '4px' }}>🗑️</button>
                        </td>
                      )}
                    </tr>
                  ))
            ...
            {/* FORMULÁRIO PARA ADICIONAR NOVO MEMBRO DO AGREGADO */}
            {isAdmin && (
              <form onSubmit={handleAdicionarMembro} style={{ marginTop: '16px', backgroundColor: '#eff6ff', padding: '12px', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', fontSize: '11px' }}>
                  ...
                  <input type="text" placeholder="Relação (ex: Cônjuge, Filho)" value={novoMembro.relacao} onChange={e => setNovoMembro({...novoMembro, relacao: e.target.value})} style={{ padding: '6px' }} />
